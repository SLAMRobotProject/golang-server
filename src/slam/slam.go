...
func SlamBackendLoop(
	...
	fullScanData := make([]float32, MAX_RANGER_DATA_POINTS)
	...
	for {
		...
		case newData := <-chSensorData:
			...
			if idx0 >= 0 && idx0 < MAX_RANGER_DATA_POINTS {
				fullScanData[idx0] = cartesianToDistance(newData.IRSensors.IrSensorData_0)
			}
			...
			if idx1 >= 0 && idx1 < MAX_RANGER_DATA_POINTS {
				fullScanData[idx1] = cartesianToDistance(newData.IRSensors.IrSensorData_1)
			}
			...
			if idx2 >= 0 && idx2 < MAX_RANGER_DATA_POINTS {
				fullScanData[idx2] = cartesianToDistance(newData.IRSensors.IrSensorData_2)
			}
			...
			if idx3 >= 0 && idx3 < MAX_RANGER_DATA_POINTS {
				fullScanData[idx3] = cartesianToDistance(newData.IRSensors.IrSensorData_3)
			}
			...
			if sendScan {
				...
				scanMsg.Ranges = make([]float32, MAX_RANGER_DATA_POINTS)
				copy(scanMsg.Ranges, fullScanData)
	...
	for range ticker.C {
		...
		data := T.SensorData{
			...
			IRSensors: T.IRSensors{
				...
				IrSensorData_1: T.Point2D{X: 3.0, Y: 10.0},
