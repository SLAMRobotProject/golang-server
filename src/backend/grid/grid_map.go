...
func (g GridMap) PoseToCoordinates(p pose.Pose) (int, int) {
	return util.MetresToCm(p.X), util.MetresToCm(p.Y)
}

func (g GridMap) SensorPointToCoordinates(p pose.Pose, xSensorMM, ySensorMM int) (int, int) {
...
func (g GridMap) CoordinatesToPose(x, y int) pose.Pose {
	return pose.Pose{X: util.CmToMetres(x), Y: util.CmToMetres(y), Theta: 0}
}
